...
export default function BinanceAccountSummary() {
  ...
  if (error) {
    ...
    const isAuthError = err?.httpStatus === 401 || err?.httpStatus === 403 || err?.code === -2015;
    return (
      <div className="flex items-start gap-2 p-3 bg-destructive/10 rounded border border-destructive/30">
        ...
        <div>
          <p className="text-xs text-destructive font-mono font-bold">
            {isAuthError ? 'Chave inválida ou sem permissão' : 'Erro ao buscar conta'}
          </p>
    ...
    <div className="space-y-2">
      <div className="flex items-center justify-between">
        <div className="flex items-center gap-1.5">
          ...
          <span className="text-xs font-mono text-muted-foreground">Conta Futures</span>
        </div>
        <Button
          size="sm"
          variant="ghost"
      ...
      <div className="grid grid-cols-3 gap-2">
        <div className="bg-secondary/50 rounded p-2">
          <p className="text-2xs text-muted-foreground font-mono">Saldo Total</p>
        ...
        <div className="bg-secondary/50 rounded p-2">
          <p className="text-2xs text-muted-foreground font-mono">PnL Não Real.</p>
          <p className={`text-sm font-mono font-bold ${pnlPositive ? 'text-[var(--trading-green)]' : 'text-[var(--trading-red)]'}`}>
            {pnlPositive ? '+' : ''}{data.totalUnrealizedProfit.toFixed(2)}
          </p>
          <p className="text-2xs text-muted-foreground font-mono">USDT</p>
        </div>
      </div>
