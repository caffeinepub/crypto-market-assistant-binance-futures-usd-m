...
export default function CandlestickChart({
  ...
  useEffect(() => {
    ...
    const toY = (price: number) =>
      CHART_PADDING.top + chartHeight - ((price - priceMin) / priceSpan) * chartHeight;
    ...
    for (const fvg of smcAnalysis.fairValueGaps) {
      ...
      const fvgIdx = fvg.index - (visibleCandles.length > 0 ? candles.length - visibleCandles.length : 0);
    ...
    for (const lz of smcAnalysis.liquidityZones) {
      ...
      if (lzY < CHART_PADDING.top || lzY > height - CHART_PADDING.bottom) continue;
    ...
    if (indicators?.bollingerBands) {
      ...
      for (let i = 0; i < visibleCandles.length; i++) {
        ...
        if (u === null) continue;
        const x = toX(i);
        if (!started) { ctx.moveTo(x, toY(u)); started = true; }
      ...
      for (let i = 0; i < visibleCandles.length; i++) {
        ...
        if (l === null) continue;
        const x = toX(i);
        if (!started) { ctx.moveTo(x, toY(l)); started = true; }
    ...
    if (indicators?.movingAverages) {
      ...
      const drawMA = (values: (number | null)[], color: string, width: number) => {
        ctx.beginPath();
  ...
  if (error) {
    return (
      <div className="flex items-center justify-center h-full gap-2 text-destructive">
        ...
        <span className="text-sm font-mono">Erro ao carregar dados do gráfico</span>
      </div>
    );
  }
  ...
  return (
    <div className="flex flex-col h-full">
      ...
      <div className="flex items-center gap-1 px-3 py-2 border-b border-border">
        <span className="text-xs text-muted-foreground font-mono mr-2">TF:</span>
        {TIMEFRAMES.map((tf) => (
          <button
            key={tf.value}
        ...
        <div className="ml-auto flex items-center gap-3 text-2xs font-mono text-muted-foreground">
          <span className="flex items-center gap-1">
            <span className="w-3 h-0.5 inline-block" style={{ background: EMA20_COLOR }} />
            EMA20
          </span>
          <span className="flex items-center gap-1">
            <span className="w-3 h-0.5 inline-block" style={{ background: EMA50_COLOR }} />
            EMA50
          </span>
          <span className="flex items-center gap-1">
            <span className="w-3 h-0.5 inline-block" style={{ background: EMA200_COLOR }} />
            EMA200
          </span>
          <span className="flex items-center gap-1">
            <span className="w-3 h-0.5 inline-block" style={{ background: BB_LINE_COLOR }} />
            BB
          </span>
        </div>
      </div>
