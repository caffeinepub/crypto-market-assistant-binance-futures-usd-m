...
const MODALITIES: TradingModality[] = [
  "scalping",
  "swing",
  "breakout",
  "reversal",
...
const MODALITY_CONFIG: Record<
  TradingModality,
  { tpMult1: number; tpMult2: number; tpMult3: number; slMult: number }
> = {
  scalping: { tpMult1: 0.003, tpMult2: 0.006, tpMult3: 0.01, slMult: 0.004 },
  swing: { tpMult1: 0.015, tpMult2: 0.03, tpMult3: 0.05, slMult: 0.012 },
  breakout: { tpMult1: 0.02, tpMult2: 0.04, tpMult3: 0.07, slMult: 0.015 },
  reversal: { tpMult1: 0.012, tpMult2: 0.025, tpMult3: 0.045, slMult: 0.01 },
  smc: { tpMult1: 0.018, tpMult2: 0.035, tpMult3: 0.06, slMult: 0.013 },
  fvg: { tpMult1: 0.01, tpMult2: 0.022, tpMult3: 0.04, slMult: 0.008 },
};
...
export async function fetchValidatedSymbols(): Promise<string[]> {
  try {
    ...
    const symbols: string[] = (data.symbols || [])
      .filter(
        (s: { contractType: string; quoteAsset: string; status: string }) =>
...
function scoreSymbolForModality(
  ...
  const volume = Number.parseFloat(ticker.quoteVolume) || 0;
  const high = Number.parseFloat(ticker.highPrice) || price;
  const low = Number.parseFloat(ticker.lowPrice) || price;
  ...
  switch (modality) {
    ...
    case "reversal":
      ...
      return (
        (Math.abs(change) > 5 ? Math.abs(change) / 20 : 0) * 0.7 + range * 0.3
      );
    case "smc":
...
async function openTradeForModality(
  modality: TradingModality,
  tickers: Map<string, BinanceTicker>,
): Promise<void> {
  ...
  const entryPrice = Number.parseFloat(ticker.lastPrice);
