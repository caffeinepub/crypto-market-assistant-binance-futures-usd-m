...
class LearningEngine {
    ...
    return new Promise((resolve, reject) => {
      ...
      request.onsuccess = (event) => {
        ...
        if (cursor) {
          ...
          if (
            Math.abs(record.timestamp - timestamp) < 24 * 60 * 60 * 1000 &&
            !record.actualPrice
          ) {
  ...
  async updateAssetStats(symbol: string): Promise<void> {
    ...
    const averageConfidence =
      completedPredictions.reduce((sum, p) => sum + p.confidence, 0) /
  ...
  private async getPredictionsForAsset(
    symbol: string,
  ...
  private async optimizeWeights(
    ...
    const avgCorrect = {
      smc:
        correctPredictions.reduce((sum, p) => sum + p.indicators.smc, 0) /
        correctPredictions.length,
      volumeDelta:
        correctPredictions.reduce(
          (sum, p) => sum + p.indicators.volumeDelta,
          0,
        ) / correctPredictions.length,
      liquidity:
        correctPredictions.reduce((sum, p) => sum + p.indicators.liquidity, 0) /
        correctPredictions.length,
      fvg:
        correctPredictions.reduce((sum, p) => sum + p.indicators.fvg, 0) /
        correctPredictions.length,
    };
    ...
    const totalAvg =
      avgCorrect.smc +
      avgCorrect.volumeDelta +
      avgCorrect.liquidity +
      avgCorrect.fvg;
    if (totalAvg > 0) {
      weights.smc =
        weights.smc * (1 - learningRate) +
        (avgCorrect.smc / totalAvg) * learningRate;
      weights.volumeDelta =
        weights.volumeDelta * (1 - learningRate) +
        (avgCorrect.volumeDelta / totalAvg) * learningRate;
      weights.liquidity =
        weights.liquidity * (1 - learningRate) +
        (avgCorrect.liquidity / totalAvg) * learningRate;
      weights.fvg =
        weights.fvg * (1 - learningRate) +
        (avgCorrect.fvg / totalAvg) * learningRate;

      // Normalize weights to sum to 1
      const sum =
        weights.smc + weights.volumeDelta + weights.liquidity + weights.fvg;
      weights.smc /= sum;
  ...
  async getHighLearningAssets(minLearningLevel = 0.6): Promise<string[]> {
  ...
  async clearOldPredictions(daysToKeep = 30): Promise<void> {
    if (!this.db) await this.initialize();
