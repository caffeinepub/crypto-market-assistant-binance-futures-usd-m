...
export function computeTradeRecommendation(
  ...
  const currentPrice = Number.parseFloat(marketData.lastPrice);
  const highPrice = Number.parseFloat(marketData.highPrice);
  const lowPrice = Number.parseFloat(marketData.lowPrice);
  ...
  const _atrPercent = atr / currentPrice;
  ...
  rationale.push(
    `${analysis.trend === "bullish" ? "Bullish" : "Bearish"} trend detected`,
  );
  ...
  if (direction === "Long") {
    ...
    if (nearestResistance) {
      ...
      rationale.push(
        `Entry near resistance at $${nearestResistance.toFixed(2)}`,
      );
    } else {
      entry = currentPrice * (1 + params.entryOffset);
  ...
  if (direction === "Long") {
    ...
    if (
      nearestSupport &&
      nearestSupport > atrStop &&
      nearestSupport > entry * 0.9
    ) {
      stopLoss = nearestSupport * 0.995; // 0.5% below support
      rationale.push(
        `Stop loss below support at $${nearestSupport.toFixed(2)}`,
      );
    } else {
      stopLoss = Math.max(atrStop, entry * (1 - params.riskPercent));
      rationale.push(
        `Stop loss based on ${(params.atrMultiplier).toFixed(1)}x ATR`,
      );
    }
  } else {
    ...
    if (
      nearestResistance &&
      nearestResistance < atrStop &&
      nearestResistance < entry * 1.1
    ) {
      stopLoss = nearestResistance * 1.005; // 0.5% above resistance
      rationale.push(
        `Stop loss above resistance at $${nearestResistance.toFixed(2)}`,
      );
    } else {
  ...
  if (direction === "Long") {
    ...
    if (nearestResistance) {
      ...
      if (analysis.strength > 70) {
        ...
        rationale.push(
          `TP targeting resistance at $${nearestResistance.toFixed(2)}`,
        );
      } else {
        // Conservative: take profit before resistance
        takeProfit = Math.min(takeProfit, nearestResistance * 0.99);
        rationale.push(
          `TP before resistance at $${nearestResistance.toFixed(2)}`,
        );
      }
    } else {
      rationale.push(
        `TP based on ${params.rewardRatio.toFixed(1)}:1 R/R ratio`,
      );
    }
  } else {
    takeProfit = entry - baseReward;
  ...
  if (analysis.institutionalOrders.length > 0) {
    ...
    if (
      ...
      rationale.push(
        `Institutional flow aligned (${(order.confidence * 100).toFixed(0)}% confidence)`,
      );
    }
  }
