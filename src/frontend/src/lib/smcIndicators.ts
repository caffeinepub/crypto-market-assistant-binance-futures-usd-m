    ...
    if (candle.close < candle.open && bodyRatio > 0.5) {
      ...
      if (bullishMove) {
        const mitigated = candles
          .slice(i + 1)
          .some((c) => c.low <= candle.low && c.high >= candle.high);
        orderBlocks.push({
          type: "bearish",
          high: candle.high,
          low: candle.open,
...
export function detectLiquidityZones(candles: Candle[]): LiquidityZone[] {
  ...
  for (const idx of swingHighIndices.slice(-10)) {
    const swept = candles
      .slice(idx + 1)
      .some((c) => c.high > candles[idx].high);
    zones.push({
      type: "swing_high",
...
export function detectPremiumDiscountZone(
  candles: Candle[],
