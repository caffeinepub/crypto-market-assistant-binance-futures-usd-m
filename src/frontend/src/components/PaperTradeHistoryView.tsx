...
export default function PaperTradeHistoryView({
  onBack,
}: PaperTradeHistoryViewProps) {
  const { closedTrades, isLoadingHistory } = usePaperTradeHistory();
  const [modalityFilter, setModalityFilter] = useState<TradingModality | "all">(
    "all",
  );
      ...
      {isLoadingHistory ? (
        ...
        <div className="text-center py-12 text-muted-foreground">
          ...
          <p className="text-xs mt-1">
            The AI engine will close trades when TP or SL is hit.
          </p>
        </div>
      ) : (
