...
import { CheckCircle, Eye, EyeOff, Key, Shield, Trash2 } from "lucide-react";
import React, { useState } from "react";
import { useBinanceAuth } from "../hooks/useBinanceAuth";

export default function BinanceCredentialsForm() {
  const { hasCredentials, saveCredentials, clearCredentials } =
    useBinanceAuth();
    ...
    <div className="space-y-4">
      ...
      <div className="flex items-start gap-2 p-3 bg-secondary/50 rounded-lg border border-border">
        ...
        <p className="text-xs text-muted-foreground">
          Suas chaves são armazenadas{" "}
          <strong className="text-foreground">
            apenas no localStorage do seu navegador
          </strong>{" "}
          e nunca são enviadas ao backend. Use chaves com permissão somente de
          leitura.
        </p>
      </div>
      ...
      {hasCredentials && (
        <div className="flex items-center gap-2 p-2 bg-[var(--trading-green)]/10 rounded border border-[var(--trading-green)]/30">
          ...
          <span className="text-xs text-[var(--trading-green)] font-mono">
