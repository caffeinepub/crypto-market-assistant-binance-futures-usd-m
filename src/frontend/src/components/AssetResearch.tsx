...
export default function AssetResearch() {
      ...
      try {
        ...
        if (response.ok) {
          ...
          const usdtSymbols = data.symbols
            .filter(
              (s: any) => s.symbol.endsWith("USDT") && s.status === "TRADING",
            )
            .map((s: any) => s.symbol)
            .sort();
  ...
  const filteredSymbols = useMemo(() => {
    ...
    return allSymbols.filter((symbol) => symbol.includes(term)).slice(0, 20); // Limit to 20 suggestions
  ...
  const selectedAsset =
    searchedAsset || marketData?.find((m) => m.symbol === selectedSymbol);
        ...
        <CardContent>
          <div className="flex items-center gap-2">
            ...
            <p className="text-muted-foreground">
              Dados não disponíveis no momento. Tente outro símbolo.
            </p>
          </div>
        </CardContent>
      </Card>
  ...
  const hasManipulationZones =
    selectedAsset.analysis.manipulationZones.length > 0;
  const hasInstitutionalOrders =
    selectedAsset.analysis.institutionalOrders.length > 0;
              ...
              <div>
                <CardTitle
                  className={cn(
                    ...
                    isPositive
                      ? "text-neon-green neon-text-bullish"
                      : "text-neon-red neon-text-bearish",
                  )}
                >
                  {selectedSymbol}
                </CardTitle>
                <CardDescription className="text-base font-medium">
                  Análise Detalhada com Inteligência Preditiva - Binance Futures
                  USD-M
                </CardDescription>
              </div>
              <Badge
            ...
            >
              ...
              <CardContent className="space-y-4">
                  ...
                  <div className="space-y-2">
                      ...
                      <div className="flex items-start gap-2">
                        ...
                        <div>
                          ...
                          <p className="text-xs text-muted-foreground">
                            Áreas de liquidez onde instituições podem estar
                            acumulando ou distribuindo posições
                          </p>
                        </div>
                      </div>
                    )}
                    {hasInstitutionalOrders && (
                      <div className="flex items-start gap-2">
                        ...
                        <div>
                          ...
                          <p className="text-xs text-muted-foreground">
                            Movimentos de grande volume indicando atividade de
                            smart money
                          </p>
                        </div>
                      </div>
                    )}
                    {selectedAsset.analysis.learningLevel &&
                      selectedAsset.analysis.learningLevel > 0.5 && (
                        <div className="flex items-start gap-2">
                          <img
                          ...
                          <div>
                            <p className="text-sm font-bold text-neon-blue">
                              ...
                              {(
                                selectedAsset.analysis.learningLevel * 100
                              ).toFixed(0)}
                              %)
                            </p>
                            <p className="text-xs text-muted-foreground">
                              Sistema desenvolveu padrões consistentes de
                              predição para este ativo
                            </p>
                          </div>
                        </div>
                      )}
                    {!hasManipulationZones && !hasInstitutionalOrders && (
                      <p className="text-sm text-muted-foreground">
                        Nenhum evento especial detectado no momento. Mercado
                ...
                <div className="p-4 rounded-lg bg-card/60 border-2 border-border">
                  ...
                  <div className="space-y-2">
                    {selectedAsset.analysis.trend === "bullish" &&
                      selectedAsset.analysis.strength > 70 && (
                        <div className="flex items-start gap-2">
                          ...
                          <div>
                            ...
                            <p className="text-xs text-muted-foreground">
                              Forte momentum de alta com alta confiança.
                              Possível continuação do movimento ascendente.
                            </p>
                          </div>
                        </div>
                      )}
                    {selectedAsset.analysis.trend === "bearish" &&
                      selectedAsset.analysis.strength > 70 && (
                        <div className="flex items-start gap-2">
                          ...
                          <div>
                            ...
                            <p className="text-xs text-muted-foreground">
                              Forte pressão vendedora com alta confiança.
                              Possível continuação do movimento descendente.
                            </p>
                          </div>
                        </div>
                      )}
                    {(selectedAsset.analysis.strength < 40 ||
                      selectedAsset.analysis.confidence < 0.4) && (
                      <div className="flex items-start gap-2">
                        ...
                        <div>
                          ...
                          <p className="text-xs text-muted-foreground">
                            Mercado sem direção clara. Possível período de
                            acumulação antes de movimento significativo.
                          </p>
                        </div>
                      </div>
                    )}
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6 mb-8">
              ...
              <div className="p-5 rounded-lg bg-accent/30 border-2 border-neon-blue/40 shadow-neon-md">
                ...
                <p className="text-3xl font-bold text-neon-blue">
                  $
                  {(
                    Number.parseFloat(selectedAsset.quoteVolume) / 1000000
                  ).toFixed(2)}
                  M
                </p>
              </div>
              ...
              <Card className="border-2 border-neon-cyan/30 bg-card/60 shadow-neon-md">
                ...
                <CardContent className="space-y-4">
                  ...
                  >
                    ...
                    <p className="text-xs text-muted-foreground">
                      Análise baseada em SMC, Volume Delta, Price Action e zonas
                      de liquidez
                    </p>
                  </div>
                  ...
                  >
                    ...
                    <div className="flex items-baseline gap-2">
                      ...
                      >
                        (
                        {(
                          ((selectedAsset.analysis.prediction -
                            Number.parseFloat(selectedAsset.lastPrice)) /
                            Number.parseFloat(selectedAsset.lastPrice)) *
                          100
                        ).toFixed(2)}
                        %)
