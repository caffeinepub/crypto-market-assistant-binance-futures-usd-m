...
import AltaPatternPanel from "./AltaPatternPanel";
...
export default function TopRecommendations() {
  ...
  const [activeView, setActiveView] = useState<"recommendations" | "alta">(
    "recommendations",
  );
    ...
    <div className="space-y-4">
      ...
      >
        <TabsList className="w-full grid grid-cols-2">
          ...
          <TabsTrigger value="alta" data-ocid="ia.alta.tab" className="gap-2">
          ...
          <Card className="border-2 border-neon-green/30 bg-card/60 backdrop-blur-sm shadow-neon-bullish">
            <CardHeader>
              <div className="flex items-center gap-2">
                ...
                <CardTitle className="text-2xl text-neon-green neon-text-bullish">
                  Top Recommendations
                </CardTitle>
              </div>
              <CardDescription className="text-base">
                Assets with highest upside potential based on advanced
                algorithmic analysis, predictive confidence and continuous
                learning
              </CardDescription>
            </CardHeader>
            <CardContent>
              {!hasRecommendations ? (
                <Alert className="border-2 border-neon-cyan/40 bg-neon-cyan/10">
                  ...
                  <AlertTitle className="text-neon-cyan font-bold">
                    Generating Recommendations
                  </AlertTitle>
                  <AlertDescription className="text-muted-foreground">
                    The system is analyzing live market data and generating
                    recommendations automatically. Recommendations will appear
                    shortly based on Smart Money Concepts, Volume Delta,
                    Liquidity analysis, institutional order detection and
                ...
                <ScrollArea className="h-[700px] pr-2 overflow-x-hidden">
                  <div className="space-y-4 min-w-0">
                    {topRecs.map((rec, index) => {
                      ...
                      const hasLearning =
                        rec.market &&
                        (rec.market.analysis.learningLevel || 0) >= 0.6;
                      return (
                        <Card
                              ...
                              <div className="flex items-center gap-3 min-w-0 flex-1">
                                ...
                                <div className="min-w-0 flex-1">
                                  <div className="flex items-center gap-2 flex-wrap">
                                    <h3 className="font-bold text-xl text-neon-green neon-text-bullish truncate">
                                      {rec.symbol}
                                    </h3>
                                    {hasLearning && (
                            ...
                            {rec.market && (
                              <>
                                ...
                                {(rec.market.analysis.manipulationZones.length >
                                  0 ||
                                  rec.market.analysis.institutionalOrders
                                    .length > 0) && (
                                  <div className="flex flex-wrap gap-2 mb-4 min-w-0">
                                    {rec.market.analysis.manipulationZones
                                      .length > 0 && (
                                ...
                                <div className="space-y-2 min-w-0">
                                  <h4 className="text-sm font-bold text-neon-cyan">
                                    Key Indicators:
                                  </h4>
                                  <div className="flex flex-wrap gap-2">
                                    {rec.market.analysis.tags.map((tag) => (
                                      <Badge
                                        key={tag}
                                        variant="outline"
                                  ...
                                  <p className="text-sm text-muted-foreground leading-relaxed break-words">
                                    Strong {rec.market.analysis.trend} trend
                                    detected with {rec.strength.toFixed(0)}%
                                    strength. Technical analysis shows{" "}
                                    {rec.market.analysis.confidence >= 0.7
                                      ? "high"
                                      : "moderate"}{" "}
                                    confidence based on price action, volume
                                    analysis, and market structure.
                                    {hasLearning &&
                                      " Enhanced by AI learning from historical predictions."}
                                  </p>
                                </div>
                              </>
                            )}
