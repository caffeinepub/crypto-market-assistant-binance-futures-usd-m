...
export default function IndicatorSubCharts({
  candles,
  indicators,
  ...
  useEffect(() => {
    ...
    for (const level of [30, 50, 70]) {
      ...
      ctx.strokeStyle =
        level === 50 ? "rgba(255,255,255,0.08)" : "rgba(255,255,255,0.05)";
    ...
    if (currentRSI !== null) {
      ctx.fillStyle =
        currentRSI >= 70 ? HIST_BEAR : currentRSI <= 30 ? HIST_BULL : TEXT;
      ctx.textAlign = "right";
      ctx.fillText(currentRSI.toFixed(1), w - 2, pad.top + 10);
    }
  }, [indicators, visibleCount, offset]);
  ...
  useEffect(() => {
    ...
    const histValues = indicators.macd.histogram.slice(
      offset,
      offset + visibleCount,
    );
    const macdValues = indicators.macd.macd.slice(
      offset,
      offset + visibleCount,
    );
    const signalValues = indicators.macd.signal.slice(
      offset,
      offset + visibleCount,
    );

    const allVals = [...histValues, ...macdValues, ...signalValues].filter(
      (v) => v !== null,
    ) as number[];
  ...
  return (
    <div className="flex flex-col gap-0 border-t border-border">
      ...
      <div
        ...
        className="h-20 w-full border-t border-border"
      >
