...
export default function TechnicalIndicatorSummary({
  indicators,
  currentPrice,
}: TechnicalIndicatorSummaryProps) {
    ...
    <div className="flex flex-wrap items-center gap-2 px-3 py-2 border-b border-border bg-card">
      ...
      {movingAverages.current.ema20 && currentPrice && (
        <>
          <div className="w-px h-4 bg-border" />
          <div className="flex items-center gap-1.5">
            <span className="text-2xs font-mono" style={{ color: "#00aaff" }}>
              EMA20
            </span>
            <span
              className={`text-2xs font-mono ${currentPrice > movingAverages.current.ema20 ? "text-[var(--trading-green)]" : "text-[var(--trading-red)]"}`}
            >
