      ...
      if (symbolSet.has(item.symbol)) {
        try {
          results.set(item.symbol, {
            symbol: item.symbol,
            fundingRate: Number.parseFloat(item.lastFundingRate || "0"),
            fundingTime: Number.parseInt(item.nextFundingTime || "0"),
            markPrice: Number.parseFloat(item.markPrice || "0"),
          });
        } catch (error) {
    ...
    for (const item of data as any[]) {
      if (symbolSet.has(item.symbol)) {
        try {
          results.set(item.symbol, {
            symbol: item.symbol,
            openInterest: Number.parseFloat(item.openInterest || "0"),
            time: Number.parseInt(item.time || Date.now()),
          });
        } catch (error) {
          console.warn(
            `Error parsing open interest for ${item.symbol}:`,
            error,
          );
        }
      }
