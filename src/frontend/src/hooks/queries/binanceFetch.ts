...
function classifyError(err: unknown, status?: number): BinanceFetchError {
  ...
  if (status === 451 || status === 403) {
    return {
      type: "blocked",
      message: "Binance is blocked or restricted in your region.",
      status,
    };
  }
  if (status === 404) {
 ...
 */
export async function fetchBinanceFuturesTicker(
  symbol: string,
 ...
 */
export async function fetchBinanceAsset(
  symbol: string,
): Promise<BinanceTicker> {
 ...
 */
export async function fetchBinanceFuturesTickers(): Promise<
  BinanceMarketData[]
> {
  ...
  return data
    ...
    .map((d) => ({
      ...
      price: Number.parseFloat(String(d.lastPrice)),
      priceChangePercent: Number.parseFloat(String(d.priceChangePercent)),
      volume: Number.parseFloat(String(d.volume)),
      quoteVolume: Number.parseFloat(String(d.quoteVolume)),
      high24h: Number.parseFloat(String(d.highPrice)),
      low24h: Number.parseFloat(String(d.lowPrice)),
      openPrice: Number.parseFloat(String(d.openPrice)),
      count: Number.parseInt(String(d.count), 10),
    }));
}
