 ...
 */
export function selectScalpingOpportunities(
 ...
 */
export function selectSwingOpportunities(
  marketData: BinanceMarketData[],
): OpportunityItem[] {
  const opportunities: OpportunityItem[] = [];
 ...
 */
export function selectBreakoutOpportunities(
  marketData: BinanceMarketData[],
): OpportunityItem[] {
  const opportunities: OpportunityItem[] = [];
  ...
  for (const asset of marketData) {
    ...
    const high = Number.parseFloat(asset.highPrice);
    const low = Number.parseFloat(asset.lowPrice);
    const _open = Number.parseFloat(asset.openPrice);
    const price = Number.parseFloat(asset.lastPrice);
 ...
 */
export function selectReversalOpportunities(
  marketData: BinanceMarketData[],
): OpportunityItem[] {
  const opportunities: OpportunityItem[] = [];
  ...
  for (const asset of marketData) {
    ...
    if (asset.analysis.institutionalOrders.length > 0) {
      ...
      const isCounterTrend =
        (priceChange > 0 && order.direction === "down") ||
        (priceChange < 0 && order.direction === "up");
      if (isCounterTrend) {
 ...
 */
export function selectSMCOpportunities(
  marketData: BinanceMarketData[],
): OpportunityItem[] {
  const opportunities: OpportunityItem[] = [];
  ...
  for (const asset of marketData) {
    ...
    if (institutionalOrders.length > 0) {
      ...
      reasons.push(
        `Institutional ${order.direction === "up" ? "buying" : "selling"} detected`,
      );
 ...
 */
export function selectFVGOpportunities(
  marketData: BinanceMarketData[],
): OpportunityItem[] {
  const opportunities: OpportunityItem[] = [];
  ...
  for (const asset of marketData) {
    ...
    const hasZones =
      asset.analysis.supportZones.length > 0 ||
      asset.analysis.resistanceZones.length > 0;
